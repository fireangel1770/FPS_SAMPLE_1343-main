...
public class FPSController : MonoBehaviour
{
    ...
    public UnityEvent AmmoRefillHit;

    public UnityEvent InteractShow;
    public UnityEvent InteractHide;
    ...
    private void OnControllerColliderHit(ControllerColliderHit hit)
    {
        ...
        if (hit.gameObject.GetComponent<AmmoRefill>())
        {
            InteractShow?.Invoke();
            if (hit.gameObject.GetComponent<AmmoRefill>() && Input.GetButtonDown("Interact"))
            {
                var collisionPoint = hit.collider.ClosestPoint(transform.position);
                var knockbackAngle = (transform.position - collisionPoint).normalized;
                velocity = (20 * knockbackAngle);
